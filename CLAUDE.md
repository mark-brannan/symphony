...
- Runbook edits land in their own commit, and `prose-budget` (at commit
  via `--staged`, and in CI) budgets prose per section and per commit. The
  thresholds are `.prose-budgets.json`; tune the numbers there rather than
  arguing with it.
