...
- `kanban.md` — Claude's working backlog: micro-tasks, blocked questions,
  ...
  back at wrap-up.
- **Write the card when the loop is found, not at session end.** By wrap-up
  the context has been compacted and the comment link, the timestamp, the
  exact wording are gone. Discovery is the last moment the evidence still
  exists — add the card to `kanban.md` right then.
- **One line per card**: a link, the action in the imperative, and `blocked:`
  plus the dependency only when it's blocked. The link is not optional — a
  card nobody but its author can act on isn't a card. Facts belong in
  `reference/`, decisions with no more work behind them belong in `log.md`;
  `kanban.md` is only for what's still in flight.
- **Cards die when done, immediately.** This is a work-in-progress list, not
  a log — don't leave a resolved card in place with a paragraph of
  after-the-fact narrative explaining how it closed. Delete the line. If the
