...
- InfluxDB 2.x is a strategic dead end. Flux is in maintenance mode and
  dropped from InfluxDB 3; InfluxData's direction is v3; on 2026-05-27
  the `influxdb:latest` Docker tag was repointed to InfluxDB 3 Core
  **[verified — docs.influxdata.com via search; the repo's pin to
  `influxdb:2.7` in `compose-influxdb.yml` dodges the tag trap]**.
...
**Stale — compute_hardware.md's HALOS description.** The project moved to
its own GitHub org (`halos-org`, 31 repos), releases images roughly
...
APT (`apt install halos-marine`) — explicitly untested on Bookworm, which
rules out layering it onto the boat's current card. Architecture today:
Raspberry Pi OS Lite arm64 base; apps are Docker containers wrapped as
Debian packages from `apt.halos.fi`; Traefik + **Authelia** (ForwardAuth
+ native OIDC) for SSO; Cockpit for admin; Homarr dashboard; path-based
routing (`https://halos.local/grafana/`), the subdomain scheme having
been dropped for Windows-mDNS reasons. Marine app store: signalk-server,
grafana, influxdb, questdb, avnav, opencpn; 144 auto-converted CasaOS
apps including Node-RED; **no ntfy package found**. An **official
OpenPlotter→HALOS migration guide with backup/restore scripts** covers
SignalK, InfluxDB, Grafana and OpenCPN. Still self-described as beta and
not feature complete: Let's Encrypt TLS, 2FA, automated backup and OTA
updates are roadmap items; TLS today is a self-signed device CA
**[all verified — github.com/halos-org/{halos,halos-pi-gen,
...
**The unpriced switching cost — the dashboards.** The five Grafana
dashboards (76 panels) are InfluxQL almost throughout
**[verified — legacy_openplotter_stack.md]**. QuestDB serves Grafana over
PGWire/SQL, so every panel query must be ported; none of it transfers.
Partial offsets: the retired `signalk-grafana` plugin's auto-built
QuestDB dashboards are preserved in
...
**[verified — compose-grafana.yml]**. This is the single largest chunk of
real work in the DB swap and should be planned as its own step, not
...
- **B1. Back up InfluxDB offline.** Two artifacts, both taken and copied
  ...
  **needs no token**, sidestepping the 401 problem. Also export the five
  Grafana dashboards as JSON (already in the repo) and the datasource
  definition. After off-boat verification, the drop authorization from
  the decisions above is active. Details in the checklist below.
- **B2. Stand up QuestDB as a compose service on the boat.** New
  ...
  README]**). This is also the boat's third container, extending the
  Dex/ntfy pattern rather than inventing one.
...
- **B4. Port the dashboards.** Stand the compose Grafana up against
  ...
  preinstalled per `compose-grafana.yml`), port the five dashboards
  panel-set by panel-set from InfluxQL to SQL, seeding from the auto-built
  QuestDB dashboards where they overlap. Native Grafana keeps running
  until the container's dashboards are usable; then stop and disable the
  native `grafana-server` (a deliberate disable is the point here — get
  Mark's explicit go-ahead in-session, per the stop-don't-disable rule).
- **B5. Retire InfluxDB.** Once B3's soak shows QuestDB capturing what
  InfluxDB captures and B4's dashboards read from it: disable
...
- **C1.** With A4's verdict and the HALPI2 purchase decision, pick the
  quadrant. What Track B built transfers either way: QuestDB data and
  dashboards (HALOS ships the same DB and Grafana), the curated plugin
  manifest, Telegraf config, and the monitoring plumbing.
...
3. **Backup (announce; stops services briefly).** Stop Grafana first if
   ...
   70ce94895bf27f4d --output-path /home/pi/influx-export/symphony.lp
   --compress` (bucket id from legacy_openplotter_stack.md; re-derive
   with `influx bucket list --host http://localhost:8086` if it
   disagrees — remember `signalk.local` doesn't resolve for Go
   binaries). Restart influxdb. `scp` both artifacts off-boat; verify
   the tarball lists and the `.lp.gz` decompresses and line-counts
   sanely *off-boat* before declaring the backup real.
4. **QuestDB feasibility probe (semi-destructive, contained).**
...
5. **Plugin install dry-run (semi-destructive).** Before any npm work in
   ...
   step 4 (localhost ports, finite retention) and confirm rows arrive
   (`curl 'http://localhost:9000/exec?query=show+tables'`). Do **not**
   uninstall `signalk-to-influxdb2` in the same session — dual-run is
   the plan.
6. **Telegraf dual-write (semi-destructive, reversible).** Add a second
...
7. **Grafana ⇄ QuestDB probe (read-mostly).** Add a PGWire datasource
   (localhost:8812) to the *native* Grafana by hand or bring up the
   compose Grafana with `--no-deps` (per the compose file's own
   warning about the 8086 port race), and prove one panel of one
   dashboard reads QuestDB. This scopes the B4 porting effort with real
