...
### Boat Pi / hardware
- [ ] **Cerbo GX: confirm SignalK's Victron client connects.** From the boat LAN on 2026-09-25 the Cerbo answers again on 22/80/443/1883/8883 (Mark's ~2026-09-20 reboot fixed it), so the unit is fine; what remains is the SignalK side once `symphony-pi` is back: `scripts/halos_swap_check.sh symphony-pi` — the `victron` line should no longer read `SYN-SENT`.
- [ ] **`symphony-pi` is down, not merely offline — since 2026-09-20 ~09:20Z.** Checked from the boat LAN 2026-09-25: the Pi is absent from ARP and a full ping sweep, while the router has WAN and the Cerbo is up, so it is not a WAN or DHCP fault. The Pi is powered from the N2K bus through the PiCAN-M's SMPS (`host/telegraf-rpi-health`); Mark suspects the PiCAN-M, and a boot that pinged once then died three minutes later fits a failing SMPS or bus power. At the boat: look at the Pi's PWR LED and the PiCAN-M LEDs, check the N2K drop has 12 V, then power the Pi from USB-C directly to split PiCAN-M from Pi. Once it boots: `ip -br link show can0`, `timeout 5 candump -n 20 can0`, `docker ps` (pypilot card), and the disk-90 % check: `docker system df` and `sudo du -xsh /var/lib/docker /var/log /home/pi/* | sort -h`.
