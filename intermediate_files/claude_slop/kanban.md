...
- [ ] **Verify the containerized pypilot against the boat's own MPU9250 and its 0.56 state** — the arm64 bench run on 2026-09-02 closed everything except the hardware match. Verified there: image builds on a 2 GB Pi 4 in **416 s**; `--device /dev/i2c-1` shows `['0x48','0x68']` inside the container, same as the host; pypilot initializes the IMU (`IMU all sensor axes verified`, rate 20) and writes its calibration to the mounted volume; `pypilot_web` returns **200** on `async mode gevent`. **What is still open: the bench IMU is an ICM-20948, the boat's is an MPU9250**, so the driver match is unproven for the boat's actual chip — and pypilot 0.71 has never been pointed at the boat's 0.56 `~/.pypilot`, which is the failure that would silently lose the compass calibration. Copy the boat's `~/.pypilot` to a bench volume and start the container against it. Image `symphony/pypilot:test` and `/home/pi/pypilot-build/` are left on `symphony-halos`; rebuild is 7 min if they are gone. Design: [reference/pypilot_containerization.md](../../reference/pypilot_containerization.md), [PR #37](https://github.com/mark-brannan/symphony/pull/37).
