...
- [ ] Delete the duplicate/stale tailnet nodes in <https://login.tailscale.com/admin/machines> — `NucBoxK12` and `NucBox_K12` are the same box registered twice, and an untagged `localhost` node is online; the duplicate pair is the exact name-collision pathology [tailnet_identity.md](../../reference/tailnet_identity.md) exists to stop.
