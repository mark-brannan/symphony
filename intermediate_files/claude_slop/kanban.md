...
- [ ] **The boat's MPU9250 is still the one untested chip for containerized pypilot.** 2026-09-03 on the HALOS bench card (ICM-20948): copied the boat's live `~/.pypilot` (0.56) onto a bench volume and started `symphony/pypilot:test` (0.71) against it — every calibration field (compass ellipsoid, accel) carried over byte-for-byte and was actually applied ("Using ellipsoid compass calibration"/"Using accel calibration" in the log), the only change was `IMUType` autocorrecting 7→14 to match the bench's actually-detected chip. So the state-compat and I2C-through-container questions are closed for any chip; what's unproven is the MPU9250 driver path itself, since the bench has never had one wired. Needs either a loaner MPU9250 on the bench or a supervised run against the boat's live `~/.pypilot` on `symphony-pi` itself (still never done). Design: [reference/pypilot_containerization.md](../../reference/pypilot_containerization.md), [PR #37](https://github.com/mark-brannan/symphony/pull/37).
