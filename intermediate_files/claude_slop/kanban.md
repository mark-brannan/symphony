...
- [ ] **PR #189 does not actually recover — do not push for merge yet.** Reconnect detection and bus re-establishment are verified on real hardware (2026-09-01), but sensor data never resumes after a bus drop; the PR converts a crash into a silent no-data state. Fix the sensor re-attach path, then re-run the SIGKILL harness. See [kanban-detail.md#evidence-gathering-for-upstream-pr-189](kanban-detail.md#evidence-gathering-for-upstream-pr-189).
