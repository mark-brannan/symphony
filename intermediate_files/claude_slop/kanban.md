...
### Boat Pi / hardware
- [ ] **Decide how the HALOS image's npm `allowScripts` block gets resolved** — it prevents native bindings compiling, so `i2c-reader`, the BME680 plugin, `signalk-instrument-light-plugin`, `signalk-notifications`, `sk-propulsion-state` and `signalk-speedtest` cannot start on the halos card. Measured 2026-09-02, evidence in [halos-b3-findings-2026-09-02.md](halos-b3-findings-2026-09-02.md). Blocks the swap; options are configuring allowScripts, baking the bindings into a derived image, or dropping the affected plugins.
...
### Infrastructure
- [ ] Give `signalk-plugin-watchdog` a `plugin-config-data` entry so it actually starts — installed and linked on the halos card but never loads without one ([findings](halos-b3-findings-2026-09-02.md)).
- [ ] Find why SignalK on halos exits code 0 every ~3 min and is restarted by systemd — `signalk-healthcheck` ruled out (no `process.exit`); cause still open ([findings](halos-b3-findings-2026-09-02.md)).
