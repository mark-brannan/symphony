...
- [ ] Approve or amend the tier-override design (seed/capture, patches per tier, secret routing, GitHub Environments) in [dev-overrides-review.md](dev-overrides-review.md) § Proposed design — the inventory spike it named is **done** (§ Spike findings): the design survives, with two amendments already folded in (autostash sync withdrawn; `capture` must diff semantically against the server's schemas, not textually). Approving unblocks building `overrides/` + `seed`.
