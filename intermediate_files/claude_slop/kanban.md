...
### Infrastructure
- [ ] **Give the boat's containers a liveness check — `dex`, `questdb`, `ntfy` have none, and neither does the host run autoheal.** This is the gap that let QuestDB spin wedged for 11 hours on 2026-09-02 (250 % CPU, frozen docker I/O counters, HTTP accepting the socket then never answering) while `docker ps` reported `Up 12 days`: `unless-stopped` only fires when a process *exits*, and a spinning process never does. Preferred over restoring the commented-out `nightly-reboot` crontab line ([RUNBOOK.md](../../RUNBOOK.md) § the heartbeat section) — Mark, 2026-09-02: "I don't like the nightly-reboot unless we really need it" — because a bounce masks the failure for up to 24 h instead of catching it in minutes, and everything else 12 days of uptime cost is already fixed or separately carded. Reuse the HALOS pattern rather than inventing one: `host/halos/` already carries a working healthcheck + autoheal setup, including the `start_period` tuning learned when a 60 s window had autoheal restarting SignalK every ~3 min against a 3-4 min cold start. Per CLAUDE.md "Changes made on a host", the compose/override changes land in `host/` in the same commit. Note the sticky-flag trap: `vcgencmd get_throttled` only clears on reboot, so it reports history, not current state.
- [ ] **HIGH — fix `pypilot_web` and re-enable it** (Mark, 2026-09-02: "high priority immediately after we are stabilized"). Disabled 2026-09-02 because it served nothing (`curl :8000` → `code=000`) while writing 720 journal lines/min — 324k/day, 60 % of all journal traffic, and the reason journald reached 1.8 GB on a root fs at 77 %. Every line is the same crash: `TypeError: wrap_socket() got an unexpected keyword argument 'allow_unsafe_werkzeug'` from `_handle_and_close_when_done` → `self.wrap_socket(client_socket, **self.ssl_args)`. Measured on the boat: pypilot 0.56 (`/usr/local/lib/python3.11/dist-packages`), Flask-SocketIO 5.3.2, Flask/Werkzeug 2.2.2, gevent 22.10.2, gevent-websocket 0.10.1, python-socketio 5.7.2, python-engineio 4.3.4. Unverified candidates, in order of cheapness: pin Flask-SocketIO < 5.3.0 (5.3.0 is where `allow_unsafe_werkzeug` was added, and it appears to be leaking into gevent's `ssl_args` instead of being consumed), or upgrade pypilot past 0.56. Re-enable with `sudo systemctl enable --now pypilot_web` and verify `curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:8000/` returns 200 and the journal rate stays near zero. `pypilot.service` itself was never touched and stayed active throughout.
...
- [ ] **Install pypilot on the HALOS bench card before the swap, per [plan B4d](halos-swap-plan.md)** — Mark put pypilot in scope for the trial 2026-09-02. HALOS ships nothing for it (checked `halos-marine-containers/apps` and the SignalK image plugin list), so it is a native host install, not a container: `pip install '.[optimize,ui,hat,web]' --break-system-packages`, then `pypilot`, `pypilot_boatimu`, `pypilot_web` units. The SignalK container is host-network, so `pypilot-autopilot-provider.json` reaches `localhost:8000` unchanged. Unverified: whether the daemon runs clean with no servo device — test with the IMU on the bench. Verify `curl localhost:8000` is 200 and the web UI shows pitch/roll moving when the Pi is tilted.
