...
- [ ] Shepherd [upstream PR #189](https://github.com/naugehyde/bt-sensors-plugin-sk/pull/189) (lazy D-Bus + reconnect-on-error) to merge — deployed on the boat, `auth_timeout` workaround gone, 10h+ crash-free under real BLE error load, but the PR's own reconnect branch has never been directly observed firing; see [kanban-detail.md#evidence-gathering-for-upstream-pr-189](kanban-detail.md#evidence-gathering-for-upstream-pr-189) for what's been tried and what's next.
