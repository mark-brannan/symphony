...
### Boat Pi / hardware
- [ ] **`symphony-pi`'s power connection is intermittent — the 2026-09-20→25 outage ended when Mark nudged the box.** That rules out a dead PiCAN-M SMPS and points at a loose connection in the N2K power path: the drop cable at the tee, the PiCAN-M's screw terminals, or the HAT's seating on the GPIO header. It will recur, and remotely it looks identical to the Pi being dead. At the boat: reseat the HAT, re-torque the N2K terminals, wiggle-test each with `vcgencmd get_throttled` running, and if it still drops, move the Pi to USB-C power and leave the HAT on the bus for CAN only.
- [ ] **`symphony-pi`: the 29 GB card is at 88 % (3.3 GB free) — the disk-90 % alarm will re-trip.** Measured 2026-09-25 with the Pi back up: `/var/lib/docker` 3.6 G, `/var/log` 1.1 G, `~/influx-export` 1.4 G (the only copy — goes home on the card, never deleted here), `~/signalk_backup_20260806.zip` 449 M, `~/migration_snapshot_20260811` 109 M. `docker system df` offers 267 M of images and ~40 M of build cache. The safe ~1.3 G is `sudo journalctl --vacuum-size=200M` plus `docker image prune`; the logs are also the record of the 2026-09-20→25 outage, so read before vacuuming.
