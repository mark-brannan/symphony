...
- The house batteries publishing nothing was diagnosed and fixed 2026-08-20, and it was not a wedged BLE controller. `bt-sensors-plugin-sk` calls `createBluetooth()` at module scope in its `index.js`, so its D-Bus socket opens during SignalK's plugin-load sweep; dbus-next runs the auth handshake from that socket's `connect` callback, which the event loop cannot dispatch until the ~45s synchronous plugin load finishes. dbus-daemon closes the unauthenticated socket at its 30s `auth_timeout` and the handshake's first byte hits a dead socket — the `write EPIPE at handshake.js:67` that has been in the log for weeks. Reproduced in isolation on the Pi with the plugin's own dbus-next: a bus opened and then held behind a 35s synchronous block fails identically, while the same call on a free loop connects in 66ms. `hci0` was `UP RUNNING` with zero errors throughout; the radio was never involved, and `After=bluetooth.service` never could have helped. Fixed by raising `auth_timeout` to 120s via `host/dbus-auth-timeout.conf` → `/etc/dbus-1/system-local.conf`; both banks came back within 85s of the next restart. Follow-ups below.
- Fix the bus lifecycle in `bt-sensors-plugin-sk` (Mark's fork, `mark-brannan/bt-sensors-plugin-sk`) and retire the dbus workaround. Two changes: move `createBluetooth()` out of module scope into `plugin.start()` behind a memoizing getter, and attach an `error` handler that reconnects with backoff instead of leaving the bus dead for the life of the process. Laziness narrows the window; the retry is the part that actually closes it, and it also covers a mid-life bus drop. Worth upstreaming — this bites anyone running the plugin on a loaded Pi. Once it lands, remove `/etc/dbus-1/system-local.conf` and its `host/` entry.
- Install `plugins/signalk-plugin-watchdog` on the boat with `bt-sensors-plugin-sk` in `expectPlugins`. It was written for this exact incident and is still not deployed, which is why a total loss of battery data surfaced only when a migration audit tripped over 79 empty tables. `host/signalk-ble-check` catches the same silence but only heals by restarting, which cannot fix a deterministic fault; the watchdog is what makes it *visible*.
- `Cerbo GX 1` in the bt-sensors config has an empty `paths` block, so it publishes nothing regardless of the D-Bus fix — the documented silent trap in `RUNBOOK.md` → "Setting up a BLE sensor". Decide what it should publish and fill the block in.
- Trim SignalK's startup so the loop isn't starved for 45s. `signalk-plugin-internet-speed` throws on every start (`speedtest: Network unreachable`), `signalk-healthcheck` watches an `n2k-can0` that doesn't exist, and `signalk-to-noforeignland` is installed twice. Swap was 199/199 full with 828 MB available when this was measured. Cutting startup below 30s removes the conditions that made the D-Bus timeout reachable at all, workaround or no workaround.
