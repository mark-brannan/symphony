...
- [ ] Decide the fate of `/etc/docker/daemon.json` `max-concurrent-downloads: 2`, set by hand on the Pi 5 bench card 2026-09-04 to flatten the image-pull peak that browned it out. It is captured nowhere in `host/` or `ansible/`, so it dies with the card — which is correct if it is a bench-supply workaround, and wrong if it should apply to the boat. Same shape as the CPU-cap card in `## Yours`; [halos-fresh-image-rebuild.md](halos-fresh-image-rebuild.md) § Power is the Pi 5's binding constraint.
- [ ] Add CPU frequency to telegraf (`scaling_cur_freq` per core) — `[[inputs.cpu]]` is utilisation only, so neither a thermal downclock nor a deliberate cap is visible in monitoring. Found 2026-09-04 when a hand-applied 1.5 GHz cap left no trace; [halos-fresh-image-rebuild.md](halos-fresh-image-rebuild.md) § Host monitoring.
- [ ] Verify whether anything **alerts** on `rpi_health.under_voltage_now`, or whether it is only recorded — recorded-but-unwatched and alerting are indistinguishable from the metric alone. The Pi 5 brownout on 2026-09-04 was diagnosed by hand despite the metric existing and working.
- [ ] Re-check whether the HALOS image really ships passwordless `sudo` for `pi` — the fresh **Pi 5** card required a password, contradicting [halos-fresh-image-rebuild.md](halos-fresh-image-rebuild.md)'s image baseline. Either the note is wrong or the Pi 5 image differs; one `sudo -n true` on a fresh Pi 4 card settles it.
- [ ] Pre-stage the QuestDB/Grafana images on the payload card before it leaves for the boat (Mark's ask, 2026-09-04) — `docker pull` the three tags directly rather than `apt install`, which drags in `marine-influxdb-container` that `roles/base` step 20 exists to purge. ~1.7 GB, needs the card booted in a Pi 4 on wired internet. blocked: card is powered down for the swap.
