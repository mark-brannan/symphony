...
- [ ] Land [PR #34](https://github.com/mark-brannan/symphony/pull/34) (host files; your three comments applied 2026-09-01). [#33](https://github.com/mark-brannan/symphony/pull/33) needs rework, scope not yet stated. [#28](https://github.com/mark-brannan/symphony/pull/28) and [#29](https://github.com/mark-brannan/symphony/pull/29) are placeholders, not for landing. #31 closed as superseded.
- [ ] Resume the [PR #25](https://github.com/mark-brannan/symphony/pull/25) live dashboard walkthrough — Navstation done and committed (`7ff9d48`), 5 to go (Electricity, System health, Navigation, Weather, Life support). State, what's verified, and what's still open: [log.md#2026-09-01/02 — PR #25 live walkthrough, session 1 of N](log.md#2026-09-0102--pr-25-live-walkthrough-session-1-of-n-navstation-only). Demo stack is left running at localhost:3100 (admin/devadmin) in worktree `grafana-dashboards-pr25-89c9f8`.
...
### Boat Pi / hardware
- [ ] **Look at the Cerbo GX** — the boat's MQTT link to it has been in SYN-SENT since 2026-09-01 21:06Z and it answers on no port (80, 443, 1883, 8883, 22) while still answering ARP and mDNS; Venus OS services off, or a wedged Cerbo. Victron data is dead on both cards until then ([execution file](halos-swap-execution-2026-09-02.md)).
- [ ] **Decide pypilot for the trial** — not on the HALOS card (plan B4d, native install untested); the boat card runs it and `pypilot-sk` publishes attitude. Trial without, or a session installs it before the swap.
- [ ] **Calm the boat before the baseline** — load average ~12 with 13 logged-in users and QuestDB at 164 % CPU on 2026-09-02; the swap check's `questdb` line cannot answer there. Kill idle sessions, then re-run `scripts/halos_swap_check.sh symphony-pi`.
- [ ] Merge [PR #34](https://github.com/mark-brannan/symphony/pull/34) before or with [PR #33](https://github.com/mark-brannan/symphony/pull/33) — the HALOS card runs #34's `host/install.sh` branch.
...
### Infrastructure
- [ ] Finish the HALOS swap prep from [handoff-pr33-swap-prep.md](handoff-pr33-swap-prep.md) — preflight end to end on the bench card, the soak, then slim PR #33. Fable 5.1, high effort.
