...
### Boat Pi / hardware
- [ ] **Cerbo GX: confirm MQTT is back after the reboot.** Mark rebooted it at the boat around 2026-09-20/21; nothing has verified the result because `symphony-pi` has had no uplink since 2026-09-20 09:20Z (card below). First contact after the Pi returns: `scripts/halos_swap_check.sh symphony-pi` — the `victron` line answers this. If it still shows `SYN-SENT` to 192.168.8.107:8883, the unit is failed or MQTT-on-LAN got reset: check Settings → Services → MQTT on LAN (SSL). History: ARP `REACHABLE`, every port closed, since 2026-09-01 21:06Z ([execution file](halos-swap-execution-2026-09-02.md)).
- [ ] **`symphony-pi` has had no uplink since 2026-09-20 ~09:20Z.** Mark rebooted the Pi, the router and the modem at the boat that night (Sat 02:14 Pacific) while reseating cables. The Pi came up and pinged healthchecks.io once, three minutes after boot, so the Pi itself booted fine; no ping followed and Tailscale has shown it offline since. So the fault is the WAN path after the router/modem reboot — a WAN cable not reseated, the modem not re-registering, or the Pi holding a dead DHCP lease — not the Pi. Nothing can be checked remotely; at the boat, confirm the router has WAN, then `ip -br a` and `tailscale status` on the Pi. (The 2026-09-21 05:33Z "up" flip on healthchecks.io is an iPad Safari hit on the ping URL, not the boat.) The alarm you saw *before* that was `disk 90% >= 90%` on the 29 GB card, `/fail` pings from 2026-09-20 00:15Z to 08:49Z — nothing off-boat can shrink it; the first session with a shell on the box runs `docker system df` and `sudo du -xsh /var/lib/docker /var/log /home/pi/* | sort -h`.
...
### Repo & tooling
- [ ] **The `main` ruleset now requires a pull request and eight status checks — `validate.yml`'s header comment and CLAUDE.md § Git hygiene both still describe the old no-required-checks, push-straight-to-main model.** The [ruleset](https://github.com/mark-brannan/symphony/rules/21060338) was updated 2026-09-04 (read it live with `gh api repos/mark-brannan/symphony/rulesets/21060338`): a `pull_request` rule with squash-only merges, plus required checks `compose-config`, `json-yaml-syntax`, `shellcheck`, `python-syntax`, `dashboards`, `secret-scan / gitleaks`, `secret-scan / trufflehog`, `secrets-encrypted`. Sessions holding the repository-role bypass push past it and GitHub only prints "Bypassed rule violations" — which is how the README pass landed on main. Ask Mark whether the ruleset or the branch-vs-main rule is the one that should change, then fix whichever loses and strike validate.yml's stale comment block.
