...
### Repo & tooling
- [ ] **Review the 12 V nav-light controller design** — [#64](https://github.com/mark-brannan/symphony/pull/64). Seven channels: three mode-select legs into the one masthead fixture, then sidelights, stern, steaming, spreaders. The fixture questions no longer gate the board — spare legs cost one channel each — but two answers still gate the firmware: how many legs the masthead unit actually has and what each does, and what it does if two legs are energised at once (the bypass block makes that reachable by hand). Worth a bench check.
- [ ] **Two tracked files disagree about draft PRs** — symphony's `CLAUDE.md` § Git hygiene says open a warranted branch's PR "as a draft, with no reviewer requested"; `~/.claude/rules/code.md` § PR ownership says never a draft, and `~/.claude/hooks/no-draft-pr.sh` enforces it. The hook won on #64. One of the two files should give.
