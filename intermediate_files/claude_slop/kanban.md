...
### Infrastructure
- [ ] [Apply the postgsail SQLite-bind fix on the boat](kanban-detail.md#postgsail-sqlite-bind-fix-blocked-on-remote-write) — root cause found and [upstream issue #68](https://github.com/xbgmsharp/signalk-postgsail/issues/68) filed 2026-08-26; the one-line patch itself is blocked here on this session's remote-write permission and the boat's slow cellular WAN (`npm` calls hung >200s). Needs either a session with boat-write permission, or Mark applying it directly.
- [ ] [Uninstall `signalk-to-influxdb2` on the boat](kanban-detail.md#uninstall-signalk-to-influxdb2-blocked-on-boat-wan) — confirmed dead (InfluxDB purged 2026-08-25), plugin still erroring every cycle. Blocked tonight: `npm uninstall --dry-run` over ssh hung >200s on the boat's cellular WAN before completing, so a live uninstall risks the same partial-tree failure mode that's already bitten this boat twice. Retry when the WAN is behaving, or do it dockside on wifi.
