...
### Infrastructure
- [ ] **Retry the SignalK reinstall on the boat — it is still down.** `npm install -g signalk-server` failed 2026-08-25 with `EIDLETIMEOUT` after 27 min and npm rolled the tree back, so `~/.npm-global/lib/node_modules/signalk-server` does not exist. Two things are already staged for the retry: 167 MB of warm `~/.npm/_cacache`, and raised timeouts now written into `~/.npmrc` (`fetch-timeout=1200000`, `fetch-retries=8`, `fetch-retry-maxtimeout=300000`). Run it as `pi`, never `sudo`, adding `--prefer-offline` so the cache is used; then write the launcher and start socket-before-service. Full procedure is in `RUNBOOK.md` § Upgrading SignalK on the boat Pi.
...
- [ ] [Fix the dead RUNBOOK anchor to "Setting up plugins on the HALOS trial Pi" and verify SignalK admin access there](kanban-detail.md#halos-pi4-signalk-admin-access-is-oidc-not-a-token) — section was never written; access is Authelia SSO through Traefik, not a bearer token — login flow itself is unverified.
