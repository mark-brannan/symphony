...
- [ ] Stop concurrent sessions clobbering the shared `/home/solace/symphony` checkout — 2026-08-26 orphaned commit `351fdb0` (same message as `dc36c63`, one minute apart) and left ten files of stale work that would have reverted eight commits if committed. Decide the mechanism: a lock file like the SignalK dev server's, or sessions working in worktrees off a fresh fetch ([log](log.md#2026-08-26--symphony-pi-npm-install-failure-and-a-shared-checkout-near-miss)).
