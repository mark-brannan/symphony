...
### Boat Pi / hardware
- [ ] **Decide the HALOS trial's known losses before the swap:** no pypilot (no HALOS package), no GitHub/Google login (Authelia is file-based), no history before the swap. Trial without them, or hold. Plan: [halos-swap-plan.md](halos-swap-plan.md); context: [system_map.md](../../reference/system_map.md).
...
- [ ] Make `host/signalk-ble-check` and `host/signalk-after-bluetooth.conf` work on both cards — pick `marine-signalk-server-container.service` when `signalk.service` is absent (plan item B4b).
...
- [ ] Clean up our fork of [bt-sensors-plugin-sk](https://github.com/mark-brannan/bt-sensors-plugin-sk) so that it has a linear history when compared with the upstream
