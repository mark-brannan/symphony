...
### Infrastructure
- [ ] Investigate `signalk-postgsail` throwing repeated unhandled `TypeError: Provided value cannot be bound to SQLite parameter 5` (`signalk-postgsail/index.js:476`, `updateDatabase`) — seen live 2026-08-26 00:30, on every delta it processes. Distinct from the known InfluxDB-plugin noise; postgsail's own local SQLite queue, not QuestDB/InfluxDB.
