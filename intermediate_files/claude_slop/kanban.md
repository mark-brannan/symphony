...
- [ ] **Bench-test the containerized pypilot on arm64** — `pypilot/Dockerfile` + `compose-pypilot.yml` build and run verified only on the amd64 dev box (no IMU). Open on the Pi: does the image build there and how long, does `/dev/i2c-1` reach the MPU9250 from inside the container, and does pypilot master (0.71) read the boat's 0.56 `~/.pypilot` state. Procedure and verification lines: `RUNBOOK.md` § pypilot in a container; design and unknowns: [reference/pypilot_containerization.md](../../reference/pypilot_containerization.md). Supersedes the native-install card that stood here: Mark decided 2026-09-02 that pypilot goes in a container on HALOS, and [plan B4d](halos-swap-plan.md) now says so. **The i2c leg is closed** (2026-09-02): Mark wired the IMU to the bench Pi, and a container run with `--device /dev/i2c-1` sees `['0x48', '0x68']` — the ADS1115 and the MPU9250 — identical to the host. Device passthrough works on arm64 against real hardware. What that does *not* cover is whether pypilot's RTIMULib driver talks to this MPU9250; that needs the built image. Coordinate before touching the bench card — other sessions use it, and it sits at 236 MB available with swap 1843/1843 full, so the build needs the stack stopped first. Now unblocked — [PR #37](https://github.com/mark-brannan/symphony/pull/37) merged 2026-09-02. Run the exact prompt at [kanban-detail.md](kanban-detail.md#bench-test-containerized-pypilot-on-halos), verbatim.
...
- [ ] Replace `ansible.builtin.apt_repository` before ansible-core 2.25 removes it — `deb822_repository` writes `influxdata.sources` rather than the `influxdata.list` both cards carry, so it needs a deliberate migration, not a swap. `deprecation_warnings = False` in `ansible/ansible.cfg` is masking the warning until then.
