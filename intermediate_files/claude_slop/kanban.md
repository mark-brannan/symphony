...
### Boat Pi / hardware
- [ ] **Power-cycle `halos-pi4` (192.168.0.193)** — wedged since ~06:2x UTC 2026-09-02, ports accept TCP but nothing completes, thrashing on swap. Sustained swap onto an SD card is the one thing here that can damage media. Cause understood and not a HALOS fault (see [preflight](halos-swap-preflight-2026-09-02.md)); no session can reach it to stop the loop.
...
- [ ] Delete the inert `plugin-config-data/signalk-plugin-watchdog.json` on the halos card and confirm the watchdog is actually running — the plugin's registered id is `plugin-watchdog` (`index.js:22`), so the live config is the pre-existing `plugin-watchdog.json`; the file I created is ignored ([findings](halos-b3-findings-2026-09-02.md)). blocked: halos unreachable.
- [ ] Confirm after the swap whether the ~3-min SignalK restart loop persists on the boat's 4 GB — root cause is the container healthcheck *succeeding but overrunning* its 10s timeout (13.2s API response measured) so `autoheal` restarts a healthy server; expected to vanish with headroom. Do **not** pre-edit the compose timeout: it is package-managed and not a conffile, so `apt upgrade` reverts it ([findings](halos-b3-findings-2026-09-02.md)).
