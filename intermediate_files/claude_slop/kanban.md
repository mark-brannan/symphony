...
- [ ] **Tonight, after the fresh-card npm build finishes: launch [handoff-halos-swap-2026-09-03-pm.md](handoff-halos-swap-2026-09-03-pm.md)** (Opus 5, medium) — swaps the payload card back, runs the new preflight, final state sync, DNS dry run and an overnight soak. Then, at the boat, [dispatch-halos-swap-day.md](dispatch-halos-swap-day.md). Second-pass review with all findings: [halos-swap-review-2026-09-03.md](halos-swap-review-2026-09-03.md).
- [ ] Approve or amend the tier-override design (seed/capture, patches per tier, autostash sync, secret routing, GitHub Environments) in [dev-overrides-review.md](dev-overrides-review.md) § Proposed design, then launch the inventory spike it names (Opus 5, medium effort, read-only, ~1 session).
...
### Infrastructure
- [ ] Capture the boat's live `signalk-healthcheck.json` into git under sops coverage (mail block is secret-shaped); the dev pin in `dev/plugin-config-overrides/` currently overrides a file that is not in the repo. First hand run of the capture step in [dev-overrides-review.md](dev-overrides-review.md) § Proposed design.
- [ ] Switch `host/boat-hourly-sync` from fetch-only to autostash-rebase pull, per [dev-overrides-review.md](dev-overrides-review.md) § Prod hotfix path. blocked: the inventory spike (which command bombed on a dirty checkout).
