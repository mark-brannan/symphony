...
|---|---|---|
| `signalk-healthcheck-override.yml` | `/etc/container-apps/marine-signalk-server-container/symphony.override.yml` | HALOS gives SignalK 150 s to answer its healthcheck; this plugin set cold-starts in 3–4 min on a Pi 4 and autoheal restarted it every ~3 min. 15 min start window, 30 s probe timeout, probe by 127.0.0.1. |
