...
- Rotate `influxdb_captain_password`, and decide who owns InfluxDB break-glass. Fixed 2026-08-14: `POST /api/v2/signin` with the `.env` credentials returned 401 because `DOCKER_INFLUXDB_INIT_USERNAME` was `admin` and no such user exists — those `INIT_` vars only apply to a *fresh* volume, and this one predates them, so the user was never created. Repointed at `captain` and sourced the password from `influxdb_captain_password`; signin now returns 204, so the last-resort path works. What's still open is that the password is **9 characters** and is now the only thing standing between a lost token and a locked-out database. Rotating it means updating InfluxDB, sops and `.env` together. Note `influxdb_init_password` is now unreferenced by `.env.j2` and should be deleted once someone confirms nothing else reads it.
