...
- Give SignalK an NMEA 2000 input. `settings.json` has zero `pipedProviders`, so the entire N2K bus is arriving at the Pi and going nowhere — including a live GNSS fix (see `reference/compute_hardware.md`). This is the single change that unlocks the most: real position instead of `signalk-fixed-position`'s dock coordinate, COG/SOG, satellites and DOP, magnetic variation, and whatever else the bus carries. The plugins that fail on "no position" today should stop failing. `can0` is up and `canboatjs` ships with the server, so this is configuration rather than new software. Check what `signalk-fixed-position` should become afterwards — it will be competing with a real source.
...
- Decide whether 15 seconds is the right hardware watchdog timeout. `host/systemd-watchdog.conf` sets `RuntimeWatchdogUSec=15s`. On 2026-08-13 it fired twice in 33 minutes — not on a hang, but on a notification storm the box would have recovered from on its own. A hard reset is not free: it risks SD corruption, and a reset landing on an `npm install` is exactly what wrecked the plugin tree once already. The argument for keeping it tight is that nobody can power-cycle this box remotely. Recommendation is 60s — long enough that transient starvation rides out, short enough that a genuine wedge still self-heals well inside a visit to the marina.
...
- Get a GNSS receiver back on the boat computer. There is none attached — `gpsd` points at a `/dev/ttyOP_gps` that doesn't exist, no serial or USB device is present at all, and `navigation.position` comes from `signalk-fixed-position` emitting a dock coordinate. So there is no real fix, no GPS time, and chrony's refclock sits unreachable; the clock survives only on an internet connection, on a box with no RTC. Decide the source first: a USB receiver on the Pi is the quick fix, a GPS on the NMEA 2000 bus is the one that also feeds the plotter and AIS. See `reference/compute_hardware.md`.
