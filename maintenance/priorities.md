...
- Teach `scripts/rotate_age_key.sh` to update per-rule key lists in `.sops.yaml`, not just the top-level `recipients:` anchor. Blocks wiring the manifest age key (generated 2026-08-12, public `age1n566m5z8e5nmhqkhxqmpd9jr2678t6l6wrzvcxrnckdjn9r2adjs55jgp9`) as a recipient of `secrets/pseudonyms.sops.yaml` alone — the point being a key that resolves an email pseudonym without also opening the boat's live credentials. Adding it today would silently keep the old working key through a rotation and make the map unreadable
