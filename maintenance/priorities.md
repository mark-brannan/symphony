...
- Get SSO working on SignalK. Caddy, Dex, Cloudflare DNS and Grafana's OAuth are all live and verified as of 2026-08-11; SignalK is the only piece missing. The service runs 2.14.4 from `/usr/lib/node_modules`, which predates OIDC — its `/signalk/v1/auth/oidc/login` route 404s even with all nine `SIGNALK_OIDC_*` variables present in the process environment. A 2.30.0 install sits unused at `/usr/local/lib/node_modules` and does support them, but hangs on `--version`. Decide whether to repoint `/home/pi/.signalk/signalk-server` at 2.30.0, upgrade in place, or wait for the machine rebuild.
- Find out what keeps stopping InfluxDB. It went down twice on 2026-08-11 — once from a stray `sudo kill`, once cleanly with no attributable cause. Telegraf buffers to memory while it's down, so host metrics are lost, which defeats the point of collecting them.
