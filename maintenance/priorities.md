...
- **Connect SignalK to `can0`.** `pipedProviders` in `~/.signalk/settings.json` is an empty list, and has been empty in every version of `signalk/settings.json` in this repo's history — SignalK has no data connection to the NMEA 2000 bus at all. The bus is not the problem: a 15-second `candump` on 2026-08-13 decoded GNSS Position Data, Position Rapid Update, COG/SOG, System Time and Sats in View, plus navigation, heading and depth PGNs. So the boat's entire N2K dataset is being thrown away, `navigation.position` is a fixed dock coordinate from a plugin, and there is no `navigation.gnss` or `navigation.datetime` at all. This is upstream of most of the SignalK/IoT backlog — dashboards, InfluxDB retention and off-boat monitoring are all recording a boat without a position. See `reference/compute_hardware.md`.
