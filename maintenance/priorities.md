...
- **Which Grafana dashboards are the QuestDB port target?** (PR #7 review
  session; comparison done 2026-08-19.) Two sets exist and they are not
  versions of each other. The comparison that was parked here has been made
  as far as the repo allows and lives in `reference/dashboard_sets.md`;
  B4 of `reference/containerization_strategy.md` links it. **Settled
  there:** the boat's five are recoverable from git — commit 02afad7 is a
  verbatim export, self-checking against `legacy_openplotter_stack.md` on
  panel counts and datasource references — so their contents no longer need
  the boat. They are Lille Ø's dashboards unmodified: of 76 panels, 35 query
  paths this boat has never published, 5 read an LTE uplink it does not
  have, 1 embeds `lille-oe-pi.local` in an iframe, and 4 are library-panel
  references carrying no model at all. That leaves 21 known-portable and 15
  unjudgeable from here. What the imported five show that the generated six
  don't comes down to six ideas, only one of which (the barometer's
  `prediction.quadrant`/`.beaufort` forecast text) rests on a path the repo
  records as present. Also settled: **neither set runs against the boat as
  it stands** — the legacy five point 158 of 162 datasource references at a
  uid that does not exist there, and the generated six read the four split
  buckets, which no log entry records as created on the boat. So "it's what
  Mark looks at today" is weaker support for the legacy set than it looked.
  **Still open, and still needs the boat:** whether the boat's Grafana holds
  `library_element` rows for the 23 referenced library panels (and what the
  4 model-less Electricity panels query); which of 15 unrecorded paths carry
  data; which side of four naming variants is live
  (`heatIndexTemperature`/`feelsLikeTemperature`,
  `humidity`/`relativeHumidity`, `wind.speedOverGround`/`wind.speedTrue` —
  a question about the *generated* set too); whether the four buckets exist
  yet; and whether anything changed aboard since the 2026-08-11 export.
  `scripts/audit_dashboard_paths.py` answers three of those and has to run
  on the boat. Blocked on the same tailnet ACL fix as everything else — a
  2026-08-19 cloud session had no ssh client and could not resolve
  `symphony-pi`. One disagreement flagged rather than resolved:
  `legacy_openplotter_stack.md` says a few Life support panels are Flux;
  the 08-11 export has no Flux anywhere, and that dashboard's one non-builder
  target is raw InfluxQL.
